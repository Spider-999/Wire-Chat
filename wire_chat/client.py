...
import socket
import threading
from gui import App, tk, ctk, filedialog
...
class Client(App):
    ...
    def open_files(self):
            # Open a file and send it to the chat
            filename = filedialog.askopenfile(initialdir='/home/device3', title='Open file')
            print(filename.name)
            with open(filename.name, 'rb') as file:
                try:
                    data = file.read(self.BYTES)
                    while data:
                        message = 'FTP:'.encode(self.FORMAT) + data
